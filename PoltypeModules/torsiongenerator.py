...
def GenerateTorsionOptInputFile(poltype,torxyzfname,torset,phaseangles,optmol,variabletorlist,mol,currentopt):
    ...
    # Case: FENNIX
    elif poltype.toroptmethod=='FENNIX':
        inputstruct=ConvertTinktoXYZ(poltype,torxyzfname,torxyzfname.replace('.xyz','_cart.xyz'))
        inputmol= opt.load_structfile(poltype,inputstruct)
        prefix=poltype.toroptmethod+'-opt-'
        postfix='.input'  
        ...
        for tor in frotors:
            newtorset.append(tor)
        toranglelist=GrabTorsionAngles(poltype,newtorset,inputmol)
        resfilename=CreateConstraintsFileFENNIX(poltype,newtorset,toranglelist,pre)
        outputname=inputname.replace(postfix,'.log')
        optxyz,cmdstr=fennix4poltype.FENNIX_OPT(poltype,inputstruct,resfilename)
        executable='echo'
...
def get_torlist(poltype,mol,missed_torsions,onlyrotbndslist,allmissing=False):
    ...
    for bond in iterbond:
        ...
        if skiptorsion==False or allmissing==True:
            ...
            poltype.nonrotbndlist[rotbndkey].append(unq)
            poltype.nonrotbndlist=FindOtherTorsAboutBond(poltype,poltype.nonrotbndlist,bond,rotbndkey,t1,t2,t3,t4)
        ...
        for tor in frotors:
            newtorset.append(tor)
        found=False
        for otor in newtorset:
            ob=otor[1]
            oc=otor[2]
            if (ob==unq[1] and oc==unq[2]) or (ob==unq[2] and oc==unq[1]):
                found=True
                break

        if found==True:
            atoms=[mol.GetAtom(i) for i in otor]
       ...
       if key in poltype.rotbndlist.keys():
           ...
           for resttors in newtorsions:
               ...
               if resttors not in variabletorlist and resttors[::-1] not in variabletorlist:
                   ...
                   if rtang<0:
                       rtang=rtang+360
                   if (((inputmol.GetAtom(rta).GetAtomicNum() == 1) or (inputmol.GetAtom(rtd).GetAtomicNum() == 1)) and allhydtors==False and allhydtorsoneside==False and nitrogenhydtor==False) and themaxrotbnds==1:
                        continue
                   else:
                       ...
                       if nitrogenhydtor==True:
                           ...
                           if nitrogentwohydtor==True:
                               ...
                               if rotbndtorescount[rotbnd]>=themaxrotbnds:
                                   continue
                       restlist.append(resttors)
                       torsiontophaseangle[tuple([rta,rtb,rtc,rtd])]=round((rtang+phaseangle)%360)
                       torsiontomaintor[tuple([rta,rtb,rtc,rtd])]=True
                       if not firsttor:
...
def FrozenBondRestraints(poltype,torset,variabletorlist,rotbndtorescount,maxrotbnds,inputmol,restlist,phaseangles,torsiontophaseangle,torsiontomaintor):
    """
    Intent:
    Input:
    Output:
    ...
    for rotkey,torsions in newdic.items():
        ...
        for resttors in newtorsions:
            ...
            if resttors not in restlist and resttors not in variabletorlist and resttors[::-1] not in variabletorlist:
                torsiontophaseangle[tuple([rta,rtb,rtc,rtd])]=round((rtang)%360)
                torsiontomaintor[tuple([rta,rtb,rtc,rtd])]=False
                restlist.append(resttors)
...
def CreatePsi4TorOPTInputFile(poltype,torset,phaseangles,optmol,torxyzfname,variabletorlist,mol,currentopt):
    ...
    for tor in list(rottors) + list(frotors):
        ...
        geometric_list.append("{'type'    : 'dihedral', 'indices' : [ %d , %d , %d , %d ], "%tuple([_-1 for _ in tor[0:4]]) + "'value' : %.4f }\n"%(rtang))
...
def CreateConstraintsFileFENNIX(poltype,torlist,toranglelist,pre):
    """
    Intent:
    Input:
    Output:
    ...
    with open(filename, 'w') as f:
      ...
      for i in range(len(torlist)):
        ...
        if tor not in TripleBondTorsions:
          angle=toranglelist[i]
          f.write(f"dihedral {' '.join([str(t) for t in tor])} {float(angle):.4f}\n")
    return filename
