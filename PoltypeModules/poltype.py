...
class PolarizableTyper():
        ...
        if self.poltypeini==True:
            ...
            for line in results:
                if '#' not in line and line!='\n':
                    ...
                    elif "waterdimerfilename" in newline:
                        self.waterdimerfilename=a
                    elif "wateratomindex" in newline:
                        self.wateratomindex=int(a)
                    elif "moleculeatomindex" in newline:
                        self.moleculeatomindex=int(a)
                    elif "minvdwradius" in newline:
                        self.minvdwradius=float(a)
                    elif "maxvdwradius" in newline:
                        self.maxvdwradius=float(a)
                    elif "minvdwdepth" in newline:
                        self.minvdwdepth=float(a)
                    elif "maxvdwdepth" in newline:
                        self.maxvdwdepth=float(a)
    ...
    def GenerateParameters(self):
        ...
        if os.path.isfile(self.vdwmissingfilename):
            missingvdwatomindextoneighbors=databaseparser.ReadDictionaryFromFile(self,self.vdwmissingfilename)
        ...
        self.optmaxcycle=6 # increase since torsion only doing 1 DFT opt
