...
def GenerateProteinTinkerXYZFile(poltype,modifiedproteinpdbname,modproidxtotypenumber,prmfilepath,proidxtoprotype,knownresiduesymbs):
    """
    Intent:
    Input:
    Output:
...
def GrabProteinParameterTypeNumbers(poltype,prmfilepath):
    """
    Intent:
    Input:
    Output:
...
def WriteNewKeyFile(poltype,atomdefs,bondprms,angleprms,torsionprms,strbndprms,opbendprms,polarizeprms,vdwprms,mpoleprms,writekey,prmfilepath):
    """
    Intent:
    Input:
    Output:
...
def ConnectSideChainToBackbone(poltype,pdbfilename):
    ...
    atomindextoposition={}
    for line in results:
        if ('ATOM' in line or 'HETATM' in line) and 'UNL' not in line: # sometimes UNL atoms from next amino acid (like an alpha carbon that does not continue on chain at end of file) needs to be removed
            atomlabel=line[12:16].rstrip().lstrip()
            atomindex=int(line[6:11].rstrip().lstrip())
            resnumber=int(line[23:26+1])
            linesplit=line.split()
    ...
    for line in results:
        if ('ATOM' in line or 'HETATM' in line) and 'UNL' not in line: # sometimes UNL atoms from next amino acid (like an alpha carbon that does not continue on chain at end of file) needs to be removed
            ...
            linesplit=line.split()
            position=[float(linesplit[6]),float(linesplit[7]),float(linesplit[8])]
            if reslabel==poltype.modifiedresiduepdbcode:
...
def GenerateModifiedProteinPoltypeInput(poltype):
    ...
    backbonesmiles='C(C=O)N'
    backboneindexes=MatchSMARTSToOBMol(poltype,backbonesmiles,polOBmol)
    poltype.dontrotbndslist=[[backboneindexes[0],backboneindexes[1]],[backboneindexes[0],backboneindexes[-1]]]
    poltype.deleteallnonqmfiles=False
...
def GenerateModifiedProteinXYZAndKey(poltype,knownresiduesymbs,modproidxs,proboundidxs,boundaryatomidxs,proOBmol,molname,modresiduelabel,proidxtoligidx,ligidxtoproidx,modmol,smarts,check,connectedatomidx,backboneindexesreference,modligidxs):
    ...
    protinkxyz,firstresnum,lastresnum=GenerateProteinTinkerXYZFile(poltype,poltype.modifiedproteinpdbname,modproidxtotypenumber,poltype.prmfilepath,proidxtoprotype,knownresiduesymbs)

    listoftorsionsforkey,listofbondsforprm,listofanglesforprm,listoftorsionsforprm=GrabAtomsForValenceTermsAcrossBoundary(poltype,ligOBmol,proOBmol,ligidxtoproidx,proboundidxs,boundaryatomidxs,modproidxs)
    ...
    if check==False:
        ...
        if not os.path.isfile(poltype.ModifiedResiduePrmPath):
            shutil.copy(poltype.prmfilepath,poltype.ModifiedResiduePrmPath)
    ...
    torsionligtypestoboundtypesforkey=GrabLigandTypeToBoundryTypeForKeyFile(poltype,listoftorsionsforkey,proidxtoligidx,ligidxtotypeidx,proidxtoprotype,poltype.prmfilepath,modproidxs,mincorenumber,maxcorenumber)
    ...
    bondprmclassestoboundclasses=GrabPrmClassToBoundryClassForPrmFile(poltype,listofbondsforprm,proidxtoligidx,ligidxtotypeidx,proidxtoprotype,poltype.prmfilepath,modproidxs,proboundidxtoprotype,prmtypetoprmclass,mincorenumber,maxcorenumber)
    
    angleprmclassestoboundclasses=GrabPrmClassToBoundryClassForPrmFile(poltype,listofanglesforprm,proidxtoligidx,ligidxtotypeidx,proidxtoprotype,poltype.prmfilepath,modproidxs,proboundidxtoprotype,prmtypetoprmclass,mincorenumber,maxcorenumber)
    torsionprmclassestoboundclasses=GrabPrmClassToBoundryClassForPrmFile(poltype,listoftorsionsforprm,proidxtoligidx,ligidxtotypeidx,proidxtoprotype,poltype.prmfilepath,modproidxs,proboundidxtoprotype,prmtypetoprmclass,mincorenumber,maxcorenumber)
    ...
    prmtypetoprmdeflines=GrabPolarizeDefinitionLinesFromPrmFile(poltype,prmtypetopoltype,poltype.prmfilepath)
    ...
    stitchatomdefs=[]
    stitchvdwprms=[]
    WriteNewKeyFile(poltype,stitchatomdefs,stitchbondprms,stitchangleprms,stitchtorsionprms,stitchstrbndprms,stitchopbendprms,stitchpolarizeprms,stitchvdwprms,stitchmpoleprms,writekey,poltype.prmfilepath)
    if check==False:
