...
def FragmentJobSetup(poltype,strfragrotbndindexes,tail,listofjobs,jobtooutputlog,fragmol,parentdir,vdwfragment,strfragvdwatomindex,onlyfittorsions,jobtoinputfilepaths):
    ...
    init_keywords += ['use_gaus', 'use_gausoptonly', 'use_psi4_geometric_opt', 'pcm_auto']
    init_keywords += ['optpcm', 'toroptpcm', 'torsppcm']
...
def GenerateFragments(poltype,mol,torlist,parentWBOmatrix,missingvdwatomsets,nonaroringtorlist):
    ...
    for torset in torlist:
        ...
        # here we need change parentindextofragindex dictionary
        # since the post_mol is usually smaller than the pre_mol
        # one can not use MatchMolFiles function or substructure match of RDKit
        # fortunately, the coordinates does not change between post and pre mols
        # below we use xyz coordinates as flag to find the right index mapping
        # -- Chengwen Liu

        pre_mol = Chem.MolFromMolFile(structfname,removeHs=False)
        post_mol = Chem.MolFromMolFile(structfnamemol,removeHs=False)
        coord2index_pre = {}
        coord2index_post = {}
        for atom in post_mol.GetAtoms():
          atom_idx = atom.GetIdx()
          coord = post_mol.GetConformer().GetAtomPosition(atom_idx) 
          x, y, z = f"{coord.x:10.4f}", f"{coord.y:10.4f}", f"{coord.z:10.4f}"
          xyzstr = ''.join([x,y,z])
          coord2index_post[xyzstr] = atom_idx
        for atom in pre_mol.GetAtoms():
          atom_idx = atom.GetIdx()
          coord = pre_mol.GetConformer().GetAtomPosition(atom_idx) 
          x, y, z = f"{coord.x:10.4f}", f"{coord.y:10.4f}", f"{coord.z:10.4f}"
          xyzstr = ''.join([x,y,z])
          coord2index_pre[xyzstr] = atom_idx
        
        pre2post = {} 
        for k, v in coord2index_pre.items():
          if k in coord2index_post.keys():
            pre2post[v] = coord2index_post[k]
         
        keys2remove = [] 
        for k,v in parentindextofragindex.items():
          if v in pre2post.keys():
            parentindextofragindex[k] = pre2post[v]
          else:
            keys2remove.append(k) 
        parentindextofragindex = {key: val for key, val in parentindextofragindex.items() if key not in keys2remove}
