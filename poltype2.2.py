...
def call_subsystem_externalnode(cmdstr,node,wait=False):
    nodeworks=True
    ...
    p = subprocess.Popen(cmd, shell=True,stdout=logfh, stderr=logfh)
    if wait==True:
        ...
        if err!=None:
            ...
            if 'No route to host' in err or rcode==-9:
                nodeworks=False
    return nodeworks
...
def parse_options(argv):
    ...
    for o, a in opts:
        if o in ("--rotalltors"):
            ...
            procfrac = a
            continue
        elif o in ("--cpunodelistfilepath"):
            cpunodelistfilepath = a
            continue
        elif o in ("--bashrcpath"):
            bashrcpath = a
            continue
        elif o in ("--torsppcm"):
            torsppcm = True
...
def initialize ():
    ...
    if not use_psi4 and not use_psi4SPonly:
        ...
        if ("GAUSS_SCRDIR" in os.environ):
            ...
            #if(float(maxdisk[:-2]) > gbfree): only use this if you start on a specific node for computation
...
def rebuild_bonds(cartmol, refmol):
    ...
    iterbond = openbabel.OBMolBondIter(refmol) # iterator for all bond objects in the molecule
...
def rm_esp_terms_keyfile(keyfilename):
    ...
    while not os.path.isfile(tmpfname):
        time.sleep(1)
        logfh.write("Waiting for "+tmpfname+'\n')
    shutil.move(tmpfname, keyfilename)
...
def is_qm_normal_termination(logfname): # needs to handle error checking now
    """
        ...
        if 'psi4' in logfname:
            for line in open(logfname):
                if "Final optimized geometry" in line or "Electrostatic potential computed" in line or 'Psi4 exiting successfully' in line:
                    ...
                    GrabFinalPsi4XYZStructure(logfname,logfname.replace('.log','_opt.xyz'))
                    term=True
        else:
            for line in open(logfname):
                if "Normal termination" in line:
                    logfh.write("Normal termination: %s\n" % logfname)
                    term=True
                if 'error' in line or 'Error' in line or 'ERROR' in line or 'impossible' in line or 'software termination' in line or 'segmentation violation':
                    error=True
...
def run_psi4(mol):
    ...
    if use_psi4SPonly==True: # try to use gaussian for opt
        ...
        term,error=is_qm_normal_termination(logoptfname)
        if not term:
        ...
        optmol=rebuild_bonds(optmol,mol)
        ...
        inputname,outputname=CreatePsi4OPTInputFile(comoptfname,comoptfname,mol)
        if term==False:
            cmdstr='psi4 '+inputname+' '+logoptfname
            call_subsystem(cmdstr,defaultnode,True)

        term,error=is_qm_normal_termination(logoptfname) # now grabs final structure when finished with QM if using Psi4
    ...
    term,error=is_qm_normal_termination(logdmafname)
    if use_psi4SPonly:
...
def run_gaussian(mol):
    ...
    if not term:
        ...
        call_subsystem_externalnode(cmdstr,defaultnode,True)
    optmol =  load_structfile(logoptfname)
    oprmol=rebuild_bonds(optmol,mol)
    term,error=is_qm_normal_termination(logdmafname)
    if not term:
        if os.path.isfile(chkdmafname):
            os.remove(chkdmafname)
        ...
        call_subsystem_externalnode(cmdstr,defaultnode,True)
    term,error=is_qm_normal_termination(logespfname)
    if espfit and not term:
...
def ConvertTinktoXYZ(filename):
    ...
    newfilename=filename.replace('.xyz_2','_minimizedcart.xyz')
...
def CreatePsi4TorOPTInputFile(comfilecoords,comfilename,b,c,consttorlist,optmol):
...
def CreatePsi4ESPInputFile(comfilecoords,mol,molecprefix,a,b,c,d,torang,phaseangle,makecube=None):
    tempread=open(comfilecoords,'r')
    results=tempread.readlines()
    tempread.close()
    comfilename= '%s-m06lsp-%d-%d-%d-%d-%03d.com' % (molecprefix,a,b,c,d,round((torang+phaseangle)%360))
    inputname=comfilename.replace('.com','_psi4.dat')
    temp=open(inputname,'w')
    temp.write('molecule { '+'\n')
    temp.write('%d %d\n' % (mol.GetTotalCharge(), 1))
...
def SplitJobs(genobject):
    ls=[]
    for i in genobject:
        ls.append(i)
    return ls

def GenerateNodeToJobListDic(joblist,cpunodes):
    if len(cpunodes)>len(joblist): # we can just run 1 job on every node in this case
        jobspernode=1
        genobject=chunker_list(list(joblist), int(len(joblist)/jobspernode))
        splitjobs=SplitJobs(genobject)
        nodetojoblist={}
        for i in range(len(splitjobs)): # in this case we have more nodes than jobs, so outerforloop is jobs
            cpunode=cpunodes[i]
            splitjob=splitjobs[i]
            nodetojoblist[cpunode]=splitjob
    else:
        jobspernode=int(len(joblist)/len(cpunodes)) # in this case we dont have enough nodes, so some nodes will have to wait for job to finish and then submit another
        genobject=chunker_list(list(joblist), int(len(joblist)/jobspernode))
        splitjobs=SplitJobs(genobject)
        nodetojoblist={}
        for i in range(len(cpunodes)): # only case we have to worry about now is if there number of jobs per node is not integer, than extra can just go on the last node
            cpunode=cpunodes[i]
            splitjob=splitjobs[i]
            nodetojoblist[cpunode]=splitjob
        # now check for any extra jobs not evenly divided
        lastnode=cpunodes[-1]
        for splitjob in splitjobs:
            if splitjob not in nodetojoblist.values():
                nodetojoblist[lastnode].extend(splitjob) # extend not append
    return nodetojoblist


def CheckIfPreviousJobsHaveFinished(previousjobs,optinputfilestoinfo,optoutputtospoutput): # handles the case where there are no previous jobs then the boolean is true
    previousjobsfinished=True
    for job in previousjobs:
        dic=optinputfilestoinfo[job]
        outputname=dic['output']
        term,error=is_qm_normal_termination(outputname)
        if term==False:
            previousjobsfinished=False
        else: # now need to check if the SP job has finished
            splog=optoutputtospoutput[outputname]
            spterm,error=is_qm_normal_termination(outputname)
            if spterm==False:
                previousjobsfinished=False
    return previousjobsfinished

def ExecuteJobsParallel(optinputfilestoinfo,optmol,a,b,c,d,torang,consttorlist,torsionrestraint,cpunodetousableproc,cpunodetousableram,cpunodetousabledisk,cpunodetodiskunit,cpunodes):
    sleeptime=1 # 1 minute
    nodetojoblist=GenerateNodeToJobListDic(optinputfilestoinfo.keys(),cpunodes) # this determines how many jobs per node only based on number of nodes relative to number of jobs and not based on node resources
    optlogfiles=[]
    finishedjobs=[]
    optoutputtospoutput={}
    while len(finishedjobs)!=len(optinputfilestoinfo.keys())*2: # *2 is for opt and SP jobs, if there is an error in the log file then need to add file to list and continue so eventually can escape while loop
        for node in nodetojoblist.keys():
            jobslist=nodetojoblist[node]
            diskunit=cpunodetodiskunit[node]
            maxmem=str(cpunodetousableram[node])+'GB' # these global variables need to be changed when writing to com file
            maxdisk=str(cpunodetousabledisk[node])+diskunit
            numproc=str(cpunodetousableproc[node])+'GB'
            for i in range(len(jobslist)):
                toroptcomfname=jobslist[i] # opt job input name
                dic=optinputfilestoinfo[toroptcomfname]
                outputlog=dic['output'] # optjob output name
                cmdstr=dic['cmdstr'] # cmdstr for executing optjob
                phaseangle=optinputfilestoinfo[toroptcomfname]['phaseangle']
                if outputlog not in finishedjobs:
                    finishedopt,error=is_qm_normal_termination(outputlog) # if opt is finished now we can generate the SP input files and then submit on same node
                if finishedopt==True: # then check if SP has been submitted or not
                    if outputlog not in optoutputtospoutput.keys(): # then I have not submitted SP job and now I can
                        finishedjobs.append(outputlog)
                        if use_psi4:
                            finalstruct=outputlog.replace('.log','_opt.xyz')
                            cartxyz,torxyzfname=tinker_minimize_QM_Struct(molecprefix,a,b,c,d,torang,optmol,consttorlist,phaseangle,finalstruct,torsionrestraint)
                            inputname,outputname=CreatePsi4ESPInputFile(finalstruct,comfilename,optmol,molecprefix,a,b,c,d,torang,phaseangle)
                            cmdstr='psi4 '+inputname+' '+outputname
                        else:
                            cartxyz,torxyzfname=tinker_minimize_QM_Struct(molecprefix,a,b,c,d,torang,optmol,consttorlist,phaseangle,outputlog,torsionrestraint)
                            inputname,outputname=GenerateTorsionSPInputFileGaus(torxyzfname,optlogfiles,molecprefix,a,b,c,d,torang,phaseangle,outputlog)
                            cmdstr = 'GAUSS_SCRDIR='+scrtmpdir+' '+gausexe+' '+inputname
                        optoutputtospoutput[outputlog]=outputname # need to keep track of SP output name to check if this finished

                        finishedsp,error=is_qm_normal_termination(outputname)
                        if finishedsp==True:
                            finishedjobs.append(outputname)
                        elif finishedsp==False and error==True: # SP crashes here
                            finishedjobs.append(outputname)
                        elif finishedsp==False and error==False:
                            call_subsystem_externalnode(cmdstr,node)
                    else: # now need to check if sp job is finished
                        splogname=optoutputtospoutput[outputlog]
                        if splogname not in finishedjobs:
                            finishedsp,error=is_qm_normal_termination(splogname)
                            if finishedsp==True:
                                finishedjobs.append(splogname)
                            elif finishedsp==False and error==True: # SP crashes here
                                finishedjobs.append(splogname)
                            elif finishedsp==False and error==False:
                                logfh.write('Waiting on '+splogname+' '+'for normal termination '+'\n')
                                logfh.flush()
                                os.fsync(logfh.fileno())
                elif finishedopt==False and error==True:
                    finishedjobs.append(outlog) # need to append twice because if opt crashed then SP will not work, splogname not created unless opt finishes all the way
                    finishedjobs.append(outlog)
                elif finishedopt==False and error==False:
                    logfh.write('Waiting on '+outputlog+' '+'for normal termination '+'\n')
                    logfh.flush()
                    os.fsync(logfh.fileno())
                    if len(jobslist)==1:
                        ...
                        previousjobs=jobslist[:i] # should not include the ith element
                        previousjobsfinished=CheckIfPreviousJobsHaveFinished(previousjobs,optinputfilestoinfo,optoutputtospoutput) # also checks if SP after each previous opt was finished, always submit in pairs opt, then SP
                        if previousjobsfinished==True:
    ...
    logfh.write('ALL QM IS FINISHED'+'\n')                
    return optlogfiles



def CreateGausTorOPTInputFile(molecprefix,a,b,c,d,phaseangle,torang,optmol,torxyzfname,consttorlist):
    ...
    gen_torcomfile(toroptcomfname,numproc,maxmem,optmol,torxyzfname) # prevstruct is just used for Atomic Num and charge,torxyzfname is used for xyz coordinates
    ...
    tmpfh.write("\n")
    tmpfh.close()
    append_basisset(toroptcomfname,optmol.GetSpacedFormula(),optbasisset)
    outputname= os.path.splitext(toroptcomfname)[0] + '.log'
    return toroptcomfname,outputname
    

def GenerateTorsionOptInputFiles(listofstructurestorunQM,molecprefix,a,b,c,d,torang,phaseanglelist,optmol,consttorlist):
    optinputfilestoinfo={}
    for i in range(len(listofstructurestorunQM)):
        ...
        if use_psi4==True and use_psi4SPonly==False:
            inputname,outputname=CreatePsi4TorOPTInputFile(toroptcomfname,toroptcomfname,b,c,consttorlist,optmol)
            cmdstr='psi4 '+inputname+' '+outputname
        else:
            inputname,outputname=CreateGausTorOPTInputFile(molecprefix,a,b,c,d,phaseangle,torang,optmol,torxyzfname,consttorlist)
            cmdstr='GAUSS_SCRDIR='+scrtmpdir+' '+gausexe+' '+inputname
        if inputname not in optinputfilestoinfo.keys():
            optinputfilestoinfo[inputname]={}
        
        optinputfilestoinfo[inputname]['output']=outputname
        optinputfilestoinfo[inputname]['cmdstr']=cmdstr
        optinputfilestoinfo[inputname]['phaseangle']=phaseangle
    return optinputfilestoinfo

def GenerateTorsionSPInputFileGaus(torxyzfname,optlogfiles,molecprefix,a,b,c,d,torang,phaseangle,prevstrctfname):
    ...
    gen_torcomfile(torspcomfname,numproc,maxmem,prevstruct,torxyzfname)
...
def tinker_minimize_angles(molecprefix,a,b,c,d,optmol,consttorlist,phaseanglelist,prevstrctfname,torsionrestraint,torang):
    ...
    for phaseangle in phaseanglelist: # we need to send back minimized structure in XYZ (not tinker) format to load for next tinker minimization,but append the xyz_2 tinker XYZ file so that com file can be generated from that 
        prevstruct = load_structfile(prevstrctfname)
        optmol=rebuild_bonds(prevstruct,optmol)
        prevstrctfname,torxyzfname=tinker_minimize(molecprefix,a,b,c,d,optmol,consttorlist,phaseangle,torsionrestraint,prevstruct,torang,'_preQMOPT')
        tinkerstructnamelist.append(torxyzfname)
    return tinkerstructnamelist


def tinker_minimize_QM_Struct(molecprefix,a,b,c,d,torang,optmol,consttorlist,phaseangle,prevstrctfname,torsionrestraint):
    prevstruct = load_structfile(prevstrctfname) # this should be a logfile
    optmol=rebuild_bonds(prevstruct,optmol)
    cartxyz,torxyzfname=tinker_minimize(molecprefix,a,b,c,d,optmol,consttorlist,phaseangle,torsionrestraint,prevstruct,torang,'_postQMOPT')
    return cartxyz,torxyzfname

       
def tinker_minimize(molecprefix,a,b,c,d,optmol,consttorlist,phaseangle,torsionrestraint,prevstruct,torang,deisgnatexyz):
    torxyzfname = '%s-opt-%d-%d-%d-%d' % (molecprefix,a,b,c,d)
    torxyzfname+='-%03d-t%s.xyz' % (round((torang+phaseangle)%360),deisgnatexyz)
    tmpkeyfname = 'tmp-%d-%d-%d-%d' % (a,b,c,d)
    tmpkeyfname+='-%03d-t%s.key' % (round((torang+phaseangle)%360),deisgnatexyz)
    ...
    call_subsystem_externalnode(mincmdstr,defaultnode,True)
    cartxyz=ConvertTinktoXYZ(torxyzfname+'_2')
    return cartxyz,torxyzfname+'_2'

def CheckNodeResources(nodelist):
    cpunodetousableproc={}
    cpunodetousableram={}
    cpunodetousabledisk={}
    cpunodetodiskunit={}
    for node in nodelist:
        result=CheckAvailableCPUNodeMemory(node)
        if result!=None:
            proc=result[0]
            usableswap=result[1]
            usabledisk=result[2]
            diskunit=result[3]
            cpunodetousableproc[node]=proc
            cpunodetousableram[node]=usableswap
            cpunodetousabledisk[node]=usabledisk
            cpunodetodiskunit[node]=diskunit
    return cpunodetousableproc,cpunodetousableram,cpunodetousabledisk,cpunodetodiskunit


def ForceNodeResources(nodelist):
    cpunodetousableproc={}
    cpunodetousableram={}
    cpunodetousabledisk={}
    cpunodetodiskunit={}
    for node in nodelist:
        cpunodetousableproc[node]=numprocallnodes
        cpunodetousableram[node]=maxmemallnodes
        cpunodetousabledisk[node]=maxdiskallnodes
        cpunodetodiskunit[node]='GB'
    return cpunodetousableproc,cpunodetousableram,cpunodetousabledisk,cpunodetodiskunit

def CheckAvailableCPUNodeMemory(node):
    outputpath=os.getcwd()+r'/'
    sshtimeout=15
    cmdstr="nproc > "+outputpath+"NprocResults.txt"
    nodeworks=call_subsystem_externalnode(cmdstr,node,True)
    if nodeworks==False:
        return None
    j=0
    while not os.path.isfile(outputpath+'NprocResults.txt'):
        if j>=sshtimeout:
            logfh.write('This node is not reachable '+node+'\n')
            return None
        time.sleep(1)
        j+=1
    f=open(outputpath+'NprocResults.txt','r')
    lines=f.readlines()
    for line in lines:
        if len(line)!=0:
            totalnumproc=line
    ...
    cmdstr="top -n1 -b > "+outputpath+"TopResults.txt"
    nodeworks=call_subsystem_externalnode(cmdstr,node,True)
    if nodeworks==False:
        return None
    j=0
    while not os.path.isfile(outputpath+'TopResults.txt'):
        if j>=sshtimeout:
            logfh.write('This node is not reachable '+node+'\n')
            return None
        time.sleep(1)
        j+=1
    f=open(outputpath+'TopResults.txt','r')
    lines=f.readlines()
    cpusum=0
    results=[]
    for x in lines:
    ...
    for i in range(len(results)-1):
        row=results[i]                    
        cpuusage=float(row[8])
        cpusum+=cpuusage
        user=row[1]
        if user not in usertocpuusage.keys():
            usertocpuusage[user]=cpuusage
        elif user in usertocpuusage.keys():
            usertocpuusage[user]+=cpuusage
    temp=float(totalnumproc)-int(cpusum/100) 
    procnum=int(temp*procfrac)

    cmdstr='df -h > '+outputpath+'DiskSpaceResults.txt'
    nodeworks=call_subsystem_externalnode(cmdstr,node,True)
    if nodeworks==False:
        return None
    j=0
    while not os.path.isfile(outputpath+'DiskSpaceResults.txt'):
        if j>=sshtimeout:
            logfh.write('This node is not reachable '+node+'\n')
            return None
        time.sleep(1)
        j+=1
    f=open(outputpath+'DiskSpaceResults.txt')
    for line in f.readlines():
        if '/scratch' in line:
            newline=line.split()
            if 'G' in newline[2]:
                availdiskmem=newline[2].replace('G','')
                diskunit='GB'
            elif 'M' in newline[2]:
                availdiskmem=newline[2].replace('M','')
                diskunit='MB'
            elif 'T' in newline[2]:
                availdiskmem=newline[2].replace('T','')
                diskunit='TB'
            elif 'K' in newline[2]:
                availdiskmem=newline[2].replace('T','')
                diskunit='KB'
    ...
    f.close()
    cmdstr='free -g > '+outputpath+'SwapResults.txt'
    nodeworks=call_subsystem_externalnode(cmdstr,node,True)
    if nodeworks==False:
        return None
    j=0
    while not os.path.isfile(outputpath+'SwapResults.txt'):
        if j>=sshtimeout:
            logfh.write('This node is not reachable '+node+'\n')
            return None
        time.sleep(1)
        j+=1
    f=open(outputpath+'SwapResults.txt','r')
    foundresults=False
    results=f.readlines()
    for lineidx in range(len(results)):
        line=results[lineidx]
        if 'buffers/cache' in line and lineidx!=0:
            foundresults=True
            newline=line.split()
            availswapmem=float(newline[3])
    ...
    if foundresults==False:
        for line in results:
            if 'Mem:' in line:
                foundresults=True
                newline=line.split()
                availswapmem=float(newline[6])
    f.close()
    availswap=float(availswapmem)
    availdisk=float(availdiskmem)
    usabledisk=float(availdisk*diskfrac)
    usableswap=float(ramfrac*availswap)
    availproc=procnum
    if os.path.isfile(outputpath+'DiskSpaceResults.txt'):
        os.remove(outputpath+'DiskSpaceResults.txt')
    if os.path.isfile(outputpath+'SwapResults.txt'):
        os.remove(outputpath+'SwapResults.txt')
    if os.path.isfile(outputpath+'TopResults.txt'):
        os.remove(outputpath+'TopResults.txt')
    if os.path.isfile(outputpath+'NprocResults.txt'):
        os.remove(outputpath+'NprocResults.txt')
    if availproc>=4:
        proc=4
    else:
        proc=availproc
    ...
    logfh.write("Node: " +node+ " Usable DiskMem: " + str(round(usabledisk))+diskunit + " Usable SwapMem: " + str(round(usableswap))+"GB"  + " Available Processors: " + str(availproc)+ " RAM Frac: "+ str(ramfrac)+" Disk Frac: "+ str(diskfrac)+" Proc Frac: "+ str(procfrac)+'\n')
    return proc,usableswap,usabledisk,diskunit

def gen_torsion(optmol,torsionrestraint,cpunodetousableproc,cpunodetousableram,cpunodetousabledisk,cpunodetodiskunit,cpunodes):
    """
    ...
    rmstr='rm *.xyz_2'
    call_subsystem_externalnode(rmstr,defaultnode)
    ...
    for tor in torlist:
        ...
        call_subsystem_externalnode(cmd,defaultnode,True)
        time.sleep(5)
        ...
        clock=[0]+list(clock)
        listoftinkertorstructuresclock=tinker_minimize_angles(molecprefix,a,b,c,d,optmol,consttorlist,clock,prevstrctfname,torsionrestraint,torang)

        listoftinkertorstructurescounterclock=tinker_minimize_angles(molecprefix,a,b,c,d,optmol,consttorlist,counterclock,prevstrctfname,torsionrestraint,torang)
        ...
        optinputfilestoinfo=GenerateTorsionOptInputFiles(listofstructurestorunQM,molecprefix,a,b,c,d,torang,fullrange,optmol,consttorlist)
        ExecuteJobsParallel(optinputfilestoinfo,optmol,a,b,c,d,torang,consttorlist,torsionrestraint,cpunodetousableproc,cpunodetousableram,cpunodetousabledisk,cpunodetodiskunit,cpunodes)
...
def gen_esp_grid(mol):
    ...
    if not os.path.isfile(espgrdfname):
        ...
        call_subsystem_externalnode(gengridcmd,defaultnode,True)
    #    shutil.move(xyzoutfile,espgrdfname)
    # Run CUBEGEN
    if use_psi4 or use_psi4SPonly:
        import psi4
        shutil.copy(espgrdfname, 'grid.dat') # same as .grid file (x,y,z) coords
        inputname,outputname=CreatePsi4ESPInputFile('optimized.xyz',comespfname,mol,True)
    ...
    if not os.path.isfile(qmesp2fname):
        ...
        call_subsystem_externalnode(genqmpotcmd,defaultnode,True)
...
#                m = re.search(r'E\(RM06L\) =\s+(\-*\d+\.\d+)',line)
    ...
    print('energy_list ',energy_list) # TEMP
...
def compute_mm_tor_energy(mol,a,b,c,d,startangle,phase_list = None,keyfile = None):
    ...
    for phaseangle in phase_list:
        ...
        call_subsystem_externalnode(mincmdstr,defaultnode,True)
        call_subsystem_externalnode(alzcmdstr,defaultnode,True)
        ...
        call_subsystem_externalnode(renamecmdstr1,defaultnode)
        call_subsystem_externalnode(renamecmdstr2,defaultnode)
        call_subsystem_externalnode(copycmdstr3,defaultnode)
        call_subsystem_externalnode(copycmdstr4,defaultnode)
...
def ReadCPUNodeList():
    cpunodelist=[]
    if os.path.isfile(cpunodelistfilepath):
        temp=open(cpunodelistfilepath,'r')
        results=temp.readlines()
        for line in results:
            if '#' not in line:
                newline=line.replace('\n','')
                linesplit=newline.split()
                cpunodelist.append(linesplit[0])
...
def main():
    ...
    if espfit:
        ...
        if not os.path.isfile(key3fname):
            ...
            call_subsystem_externalnode(optmpolecmd,defaultnode,True)
    else:
    ...
    if dontdotor==True:
        sys.exit()
    if cpunodelistfilepath!=None:
        cpunodes=ReadCPUNodeList()
    
        if numprocallnodes!=None and maxdiskallnodes!=None and maxmemallnodes!=None:
            cpunodetousableproc,cpunodetousableram,cpunodetousabledisk,cpunodetodiskunit=ForceNodeResources(cpunodes)
        else:
            cpunodetousableproc,cpunodetousableram,cpunodetousabledisk,cpunodetodiskunit=CheckNodeResources(cpunodes)
    else:
        # need to make this also do Parallel but with only one node (Default Behaviour)
        cpunodetousableproc={}
        cpunodetousableram={}
        cpunodetousabledisk={}
    ...
    call_subsystem_externalnode(cmd,defaultnode)
    ...
    call_subsystem_externalnode(grepcmd,defaultnode,True)
    logfh.flush()
    os.fsync(logfh.fileno())
